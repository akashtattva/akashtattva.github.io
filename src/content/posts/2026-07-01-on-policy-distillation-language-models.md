...
![Figure 4: Effect of divergence on quality vs diversity](/assets/img/gkd_fig4_divergence_fig5_rl.webp)

*Figure 4 shows the trade-off between generation quality and diversity for different divergences. Mode-seeking divergences (reverse KL, JSD with high beta) produce better quality but less diversity at high temperatures; the gap narrows with greedy sampling.*
...
![Figure 1: GKD outperforms KD approaches across student model sizes](/assets/img/gkd_fig1_main_results.webp)

*Figure 1 shows GKD consistently outperforming supervised KD and SeqKD across T5-small, T5-base, and T5-large students on XSum summarization. A T5-small distilled with GKD surpasses the few-shot performance of PaLM (540B).*

![Figure 3: Data efficiency of GKD](/assets/img/gkd_fig2_results_fig3_data.webp)

*Figure 3 shows on-policy GKD on just 5% of XSum data outperforms supervised KD and ImitKD using the entire dataset with ground-truth summaries.*
...
![Figure 6: Translation ablation on WMT en->de](/assets/img/gkd_fig5_rl_fig6_translation.webp)

*Figure 6 shows GKD ablations on WMT English-to-German translation. Purely on-policy data and generalized JSD divergences perform best, with the largest gains when the student is much smaller than the teacher.*
...
![Figure 9: CoT reasoning distillation on GSM8K](/assets/img/gkd_fig7_reasoning_fig9_cot.webp)

*Figure 9 shows on-policy GKD substantially outperforming other approaches on GSM8K with few-shot CoT prompting. A T5-small distilled with GKD approaches the performance of much larger models, and student-generated CoT sequences beat fixed datasets with teacher-generated CoTs.*
...
![Figure 10: Task-agnostic distillation on FLAN](/assets/img/gkd_fig10_task_agnostic.webp)

*Figure 10 shows on-policy GKD with reverse KL outperforming supervised KD and ImitKD on held-out MMLU and BBH benchmarks after task-agnostic distillation on the FLAN dataset.*
...
![Figure 5: RLAIF + On-policy GKD](/assets/img/gkd_fig4_divergence_fig5_rl.webp)

*Figure 5 shows the trade-off between reward maximization and summarization performance on XSum. GKD combined with RL fine-tuning substantially improves factual consistency while maintaining summarization quality, outperforming both the teacher and the RL-only approach.*
