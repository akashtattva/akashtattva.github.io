...
*Figure 5 shows the wall-clock time per training step for SDPO vs GRPO. SDPO adds only ~5.8â€“17.1% overhead, with the majority of time spent on response generation (Gen) rather than the self-teacher log-prob computation.*
...
*Figure 6 (left) shows SDPO achieving higher accuracy faster than GRPO on Chemistry tasks. (Right) Response lengths over training â€” SDPO maintains significantly shorter generations.*
...
*Figure 8 compares final LCBv6 accuracy across Qwen3 model sizes. SDPO's advantage over GRPO grows with scale â€” the self-teacher's retrospection ability is an emergent phenomenon.*
...
*Figure 9 visualizes the per-token advantages assigned by the self-teacher for the example from Figure 4. Blue tokens receive positive advantage, red tokens negative â€” the model learns exactly which tokens caused the error.*
